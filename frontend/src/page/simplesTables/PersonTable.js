...
import { DELETE_PERSON_MUTATION } from "../../graphql/mutations/person";
import { PERSONS_QUERY } from "../../graphql/queries/all";
...
const PersonTable = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "birth_day",
      render: (passport) =>
        passport?.birth_date ? formatDate(passport?.birth_date) : null,
    },
    {
      title: "Договор",
      key: "btnContract",
  ...
  return (
    <div>
        ...
        <Form.Item label="Поиск:" name="search">
          <Space>
            ...
            <StyledButtonGreen
              ...
              onClick={() =>
                setPersonModalStatus({ person_id: null, mode: "add" })
              }
      ...
      <Table
        ...
        loading={loading}
        dataSource={data?.persons?.items?.map((person, index) => ({
          ...person,
          key: index,
        }))}
        columns={columns}
                ...
                <Col span={7}>
                  ...
                  >
                    <Descriptions.Item label="Серия и номер">
                      {record?.passport?.serial} №{record?.passport?.number}
                    </Descriptions.Item>
                    <Descriptions.Item label="Адрес проживания">
                      {record?.passport?.address_residential}
                    </Descriptions.Item>
                    <Descriptions.Item label="Адрес регистрации">
                      {record?.passport?.address_registration}
                    </Descriptions.Item>
                    <Descriptions.Item label="Дата и место выдачи">
                      {record?.passport?.date
                        ? formatDate(record?.passport?.date)
                        : ""}{" "}
                      - {record?.passport?.passport_place_issue?.name}
                    </Descriptions.Item>
                  </Descriptions>
                </Col>
                <Col span={17}>
