...
const TemplateMenuItem = ({ isTemplate, onClick, ...props }) => {
  return isTemplate ? (
    <CustomMenuButton disabled icon={<PushpinFilled />} {...props}>
      Является шаблоном
    </CustomMenuButton>
  ) : (
    <Popconfirm
    ...
    >
      <CustomMenuButton icon={<PushpinOutlined />} {...props}>
...
const TemplateBlock = ({ record, onUpdated }) => {
  ...
  const [mutateChangeTemplate] = useMutation(CHANGE_TEMPLATE_TYPE_PROJECT, {
    ...
    onError: (error) => {
      openNotification(
        "topRight",
        "error",
        `Ошибка при изменении шаблона : ${error.message}`
      );
    },
  });
  const createTemplate = () => {
    mutateChangeTemplate({
      variables: {
        typeProject: record.type_project_document.id,
        newTemplate: record.id,
      },
    });
  };
  ...
  return (
    <>
      <TemplateMenuItem
        onClick={() => createTemplate(record.id)}
        isTemplate={
