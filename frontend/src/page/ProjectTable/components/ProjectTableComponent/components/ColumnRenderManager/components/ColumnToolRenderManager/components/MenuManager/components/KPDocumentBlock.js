...
const KPDocumentBlock = ({ project, onUpdated }) => {
  ...
  const [generateKpMutate, { loading: generateKpLoading }] = useMutation(
    GENERATED_COMMERCIAL_OFFER_MESSAGE,
    {
      onCompleted: (data) => {
        downloadFile({
          variables: { id: data.generatedCommercialOfferMessage.url },
        });
        openNotification(
          "topRight",
          "success",
      ...
      {project?.kp_file_id ? (
        ...
        <>
          ...
          <Popconfirm
            ...
            showCancel={false}
            children={<CustomMenuButton>Сгенерировать КП</CustomMenuButton>}
          />
          <LinkToDownload
            disabled={project?.project_kp_history?.length <= 0}
            fileId={
              project?.project_kp_history?.length > 0 &&
              project?.project_kp_history[
                project?.project_kp_history?.length - 1
              ].file_id
            }
          >
            <CustomMenuButton
