...
const OrganizationContactsCompactTable = ({data, refetch}) => {
        ...
        children: [
            {
                ...
                render: (text, record) => (
                    <Space.Compact direction={"vertical"} style={{alignContent: "start"}}>
                        <Text strong>{record?.last_name} {record?.first_name} {record?.patronymic}</Text>
                    </Space.Compact>
                ),
            },
            {
                ...
                render: (text, record) => (
                    <Space.Compact direction={"vertical"} style={{alignContent: "start"}}>
                        <Text strong>Телефон:  {record?.work_phone ?  record?.work_phone:
                            record?.mobile_phone ? record?.mobile_phone + " (Личный)" : "Отсутствует"}</Text>
                        <Text strong>e-mail:  {record?.work_email ?  record?.work_email:
                                        record?.email ? record?.email + " (Личный)" : "Отсутствует"}</Text>
                    </Space.Compact>
                ),
            },
            {
                ...
                render: (text, record) => (
                    <Space.Compact direction={"vertical"} style={{alignContent: "start"}}>
                        <Text strong>{record?.birth_day && formatDate(record?.birth_day)}</Text>
                    </Space.Compact>
                ),
            },
            {
            ...
            {
                title: 'Управление', key: 'edit', width: "10%", render: (text, record) => (
...
export default OrganizationContactsCompactTable;
