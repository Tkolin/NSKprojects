...
import { COMPUTE_FORMULA_MUTATION } from "../../../graphql/mutations/formula";
  ...
  const evaluateFormula = () => {
    ...
    if (emptyFieldKey === undefined) {
      ...
      openNotification(
        "topRight",
        "error",
        "Несколько переменных не могут быть неизвестны!"
      );
      return;
    }
    setLoading(true);
    ...
    <Draggable style={{ margin: 5, width: 810 }}>
      <StyledBlockBig label={formulaData?.name}>
        <Row gutter={15}>
          <Col span={12}>
            <p>
              <strong>Описание:</strong> {formulaData?.description}
            </p>
            <p>
              <strong>Формула:</strong>{" "}
              <MathComponent tex={formulaData?.latex_formula} display={false} />
            </p>
            <div>
              <Space direction="vertical" style={{ width: "100%" }}>
                {formulaData?.variable_data?.map((variable) => (
                  ...
                  <div
                    key={variable.id}
                    onDragStart={(e) =>
                      handleDragStartInput(
                        e,
                        inputColors[variable.id],
                        variable.id
                      )
                    }
