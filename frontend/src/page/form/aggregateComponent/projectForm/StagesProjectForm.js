...
const StagesProjectForm = ({project, setProject, disable, onSubmit}) => {
    ...
    const loadTemplate = () => {
        ...
        if (stages) {
            console.log("А");
            let initialValuesStages = stages.map((data) => ({
                index: data.number,
            ...
            const sumStagesDuration = stages.reduce((total, data) => total + (data.duration ?? 0), 0);

            const koef = project.duration / sumStagesDuration;
            console.log(sumStagesDuration, project.price);
            console.log(koef);
            initialValuesStages = initialValuesStages?.map((data) => ({
                ...data,
                duration_item: parseInt(data.duration_item * koef),
            }));
            console.log(koef);
            formStage.setFieldsValue({stageList: initialValuesStages});
            setTotalToDuration(project.duration);
    ...
    const [updateStagesToProject] = useMutation(UPDATE_STAGES_TO_PROJECT_MUTATION, {
        onCompleted: () => {
            ...
            const projectData = {
                ...
                date_create: project?.date_create ?? null,
                price: project?.price,
                ...
                {(fields, {add, remove}) => (<>
                    ...
                    <Space style={{
                        ...
                        <Tooltip title="Суммарная продолжительность">
                            <Space.Compact>
                                <InputNumber value={totalToDuration} disabled={disable}
                                             style={{width: 45,
                                                 background: totalToDuration > project?.duration ? '#EE4848' : totalToDuration < project?.duration ? '#FFD700' : '#7DFF7D'}}/>
                                <InputNumber value={project?.duration} disabled={disable}
                                             style={{width: 40}}/>
                            </Space.Compact>
                        </Tooltip>
