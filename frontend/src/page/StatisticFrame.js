import { AuditOutlined, DatabaseOutlined, FileAddOutlined, FileProtectOutlined, HistoryOutlined, ReconciliationOutlined } from "@ant-design/icons";
import { useQuery } from "@apollo/client";
import { Timeline, Tooltip } from "antd";
import React from "react";
import { PROJECTS_QUERY_STATISTICS } from "../graphql/queriesSpecial";
...
const App = () => {
  ...
  // Статусы проектов
  const statuses = [
    { key: "DESIGN_REQUEST", label: "Запрос на проектирование", icon: <FileAddOutlined /> },
    { key: "APPROVAL_KP", label: "Согласование КП", icon: <AuditOutlined /> },
    { key: "APPROVAL_AGREEMENT", label: "Согласование договора", icon: <FileProtectOutlined /> },
    { key: "WAITING_START_WORK", label: "Подготовка к работе", icon: <HistoryOutlined /> },
  ...
  statuses.forEach(({ key, label, icon }) => {
    ...
    filteredProjects?.forEach((project) => {
      timelineItems.push({  color: "green",children: <Tooltip title={project?.requirements?.map(row=> <>{row.comment}<br/></>)}>
