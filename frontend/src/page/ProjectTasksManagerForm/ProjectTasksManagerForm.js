...
const ProjectTasksManagerForm = ({ actualProject, onCompleted }) => {
  ...
  const findMainTask = (taskId, projectTasks) => {
    ...
    if (!startTask.project_task_inherited_id) {
      console.log(
        "!startTask.project_task_inherited_id: ",
        !startTask.project_task_inherited_id
      );
      return null;
    }
    const transparent = (task) => {
      const inheritedTask = projectTasks.find(
        (row) => row.id === task.project_task_inherited_id
      );
      if (!inheritedTask) {
  ...
  const findMainDuration = (checkedTasksId, tasks) => {
    ...
    const projectTaskInherited = tasks.find(
      (row) => row.id === taskOwner?.project_task_inherited_id
    );
        ...
        title: (
          <Space>
            <Space.Compact style={{ width: 70 }}>
              <Tooltip
                title={"Сдвижка*"}
                children={<Input size={"small"} disabled value={task.offset} />}
              />
              <Tooltip
                title={"продолжительность*"}
                children={
                  <Input size={"small"} disabled value={task.duration} />
                }
              />
            </Space.Compact>

            {task.executor ? (
              <Text style={{ color: "green" }}>{task.task.name}</Text>
            ) : (
              <Text style={{ color: "red" }}>{task.task.name}</Text>
            )}
          </Space>
        ),
        children: [],
      ...
      <Row gutter={4} style={{ width: "100%" }}>
          ...
          <Card title="Структура задач проекта" style={{ padding: "20px" }}>
            <ColoredTree
              ...
              onSelect={(selectedKeys, info) => {
                selectedKeys[0] &&
                  setCheckedTasks(
                    actualProject?.project_tasks.find(
                      (row) => selectedKeys[0] === row.id
                    ) ?? null
                  );
              }}
              draggable={true}
          ...
          {/* <Card title={"Список задач"}>
                        <Form.Item name={"tasks"}>
                            <ColoredTree
                                ...
                                // Обработка изменений, например, обновление задач после перетаскивания
                                // В этом примере просто обновляем состояние
                                setProjectTasks(newTreeData.flatMap((node) => flattenTree(node)));
                                }}
                                onSelect={(selectedKeys, info) => {
                                console.log('Выбрано:', selectedKeys, info);
                                }}
                                draggable={true}
                                selectable={true}
                                multiple={false}
        ...
        <Col span={12}>
          <TaskProjectForm
            ...
            limitDuration={findMainDuration(
              checkedTasks?.id,
