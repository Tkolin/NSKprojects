...
const TaskItem = ({ projectTasks, stage, loading }) => {
  const [progressData, setProgressData] = useState();
  const [data, setData] = useState();

  useEffect(() => {
    if (projectTasks?.length > 0 && stage) {
      const total_price = projectTasks
        .map((row) => row.price)
        .reduce(
          (acc, current) => acc + (typeof current === "number" ? current : 0),
          0
        );
      ...
      setData({
        ...
        total_task_count: projectTasks.filter(
          (second_row) => second_row.executor != null
        ).length,
  ...
  return data && progressData ? (
    ...
    >
      ...
      <Text type={"secondary"}>
        {" "}
        из <strong>{data.task_count}</strong> (доступных)
      </Text>
      ...
      {permissions.includes("read-project-payments") && (
        <>
          <Statistic
            title={"Бюджет:"}
            value={data.stage_price && formatCurrency(data.stage_price)} 
          />
          <Text type={"secondary"}>
            {" "}
            затраты: <strong>{data.total_price && formatCurrency(data.total_price)}</strong>
          </Text>
          <Progress
        size="small"
        {...progressData}
...
const TasksPriceTotal = ({ projectTasks, projectStages }) => {
  return (
    <Row gutter={1} style={{ width: "100%" }}>
      {projectStages.map((row) => (
        <Col span={projectStages.length >= 6 ? 4 : 8}>
          <TaskItem
            stage={row}
            projectTasks={
              projectTasks &&
              projectTasks.filter(
                (second_row) => second_row.stage_number === row.number
              )
            }
          />
        </Col>
