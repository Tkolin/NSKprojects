...
const GenerateOrderForm = ({ executorOrders, projectTasks, onCompleted }) => {
  ...
  const [generatedOrder, { loading: loadingGenerated }] = useMutation(
    EXECUTOR_ORDER_GENERATED,
    {
      onCompleted: (data) => {
  ...
  const rebuider = (tasks) => {
    const tasksIncludedOrder = executorOrders?.flatMap(
      (row) => row.project_tasks
    );
    return tasks
      ?.sort((a, b) => {
  ...
  return (
    <Card layout="vertical">
      <Alert
        message="Выберите задачи которые необходимо указать в договре."
        type="info"
        showIcon
      />
      <Divider />
      <Tree
        ...
        onCheck={(data) => setCheckedKeys(data)}
        treeData={rebuider(projectTasks)}
      />
      <Divider />
      <ExecutorOrderFileGenerated
        projecTaskIds={checkedKeys}
