...
export const ProjectTasksTable = ({projectTasks}) => {
    ...
    const getDateRange = (record) => {
        ...
        const dateStartText = dateStart ? dateStart.format("DD.MM.YYYY") : "Нет данных";
        const dateEndText = dateEnd ? dateEnd.format("DD.MM.YYYY") : "Нет данных";
        ...
        } else if (dateStart && dateEnd && dateStart.isBefore(dateNow) && dateEnd.isAfter(dateNow)) {
    ...
    const columns = [{
        ...
        render: (record, text) =>
            (<Space.Compact direction={"vertical"}>
                ...
                <Text><strong>{getDateRange(record)}</strong></Text>
                <Text>{record.price ? formatCurrency(record.price) : "Стоимость не указана !"}</Text>
            </Space.Compact>)
    }]
    return (
        <Table
