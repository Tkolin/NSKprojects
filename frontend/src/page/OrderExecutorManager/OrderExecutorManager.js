...
export const OrderExecutorManager = ({
  executor,
  projectId,
  projectTasks,
  onUpdated,
  ...
  const [loadOrders, { loading, data }] = useLazyQuery(EXECUTOR_ORDERS_QUERY, {
    ...
    onCompleted: (data) => {
      ...
      //onUpdated && onUpdated();
      setExecutorOrders(
        data?.executorOrders?.map((row) => ({
          ...row,
          project_tasks: [
            ...row?.project_tasks?.map((second_row) => second_row.id),
          ],
        }))
      );
    },
