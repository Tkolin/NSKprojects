...
const TaskProjectForm = ({
    ...
    const checkDisabled = () => {
        if (mainTaskToProject) setLocalDisabled(true); else setLocalDisabled(disabled);
    }
    useEffect(() => {
        checkDisabled();
    ...
    if (!taskToProject || loadingDelegates) return <Alert message="Выберите задачу" type="info" showIcon/>
    return (<Card style={{width: "100%"}}
                  ...
                  actions={[<ModalButton
                      ...
                      onClick={() => form.submit()}
                      children={`Сохранить`}/>, ...cardProps?.actions ?? []]}
                  children={<>
                      {mainTaskToProject && (
                          <Alert style={{marginBottom: 10}} message={<Text>У главной задачи уже установлен
                              исполнитель!
                              {localDisabled ? (<Link onClick={() => setLocalDisabled(false)}> хотите переназначить эту
                                  подзадачу? <br/> (нажмите
                                  для
                                  изменения)</Link>) : (<Text strong> идёт изменение</Text>)}
                      ...
                      <Form form={form} onFinish={handleComplete}
                          ...
                          <FormItem name={"offset"}
                              ...
                              <InputNumber style={{width: "100%"}} min={0}
                                           ...
                                           max={taskToProject.project_task_inherited_id ? (limitDuration - limitTimes?.duration) : 999}
                                           onChange={(value) => setLimitTimes({...limitTimes, offset: value})}
                          ...
                          <FormItem name={"duration"}
                              ...
                              <InputNumber style={{width: "100%"}} min={0}
                                           disabled={durationLock}
                                           max={taskToProject.project_task_inherited_id ? (limitDuration - limitTimes?.offset) : 999}
