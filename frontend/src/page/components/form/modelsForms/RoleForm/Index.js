...
import {
    ORGANIZATIONS_QUERY_COMPACT, PERMISSIONS_QUERY_COMPACT,
...
//     }, [organizationModalStatus]);
    ...
    const {loading: loadingPermission, error: errorPermission, data: dataPermission} = useQuery(PERMISSIONS_QUERY_COMPACT);
                ...
                <Row gutter={5}>
                    <Col span={8}>
                            ...
                            <Form.Item
                                name="description"
                                label="Описание"
                                rules={[{required: true, message: 'Пожалуйста, заполните описание'}]}
                            >
                                <Input/>
                            </Form.Item>
                    </Col>
                    <Col span={16}>
                            <PermissionsTreeComponent value={dataPermission?.permissions?.items}/>
                    </Col>
                </Row>
            </Form>
        </div>
