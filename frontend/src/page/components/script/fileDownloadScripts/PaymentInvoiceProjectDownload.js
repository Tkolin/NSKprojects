...
const PaymentInvoiceProjectDownload = ({
  projectId,
  stageNumber,
  isPrepayment,
  text,
  ...
  const [downloadProjectPaymentInvoice, { loading: loading }] = useMutation(
    PAYMENT_INVOICE_PROJECT_DOWNLOAD,
    {
      onCompleted: (data) => {
        downloadFile({
          variables: { id: data.projectPaymentInvoiceFileDownload.url },
        });
