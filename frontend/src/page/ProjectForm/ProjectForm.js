...
const defaultViews = {
  DESIGN_REQUEST: ["status_id"],
  APPROVAL_KP: ["status_id"],
  APPROVAL_AGREEMENT: ["status_id"],
  base: ["status_id"],
};
...
const ProjectForm = ({ onCompleted, project, type, options, cardProps }) => {
  ...
  const [organizationsModalStatus, setOrganizationsModalStatus] =
    useState(null);
  ...
  } = useQuery(PROJECT_COUNT_BY_ORGANIZATION, {
    variables: { organizationId: selectedOrganizationId },
  });

  const [selectedGroupTypeProject, setSelectedGroupTypeProject] =
    useState(false);
  useEffect(() => {
    !project.id && form.setFieldValue("type_project_document", null);
    if (
      dataTypeProject?.typeProjects?.items &&
      selectedGroupTypeProject &&
      !loadingTypeProject
    ) {
      setDataTypesOutput(
        dataTypeProject?.typeProjects?.items?.filter(
          (row) => row?.group?.id === selectedGroupTypeProject
        )
      );
    }
  ...
  // Логика формы
  const [mutateProject, { loading: loading }] = useMutation(
    project?.id ? UPDATE_PROJECT_MUTATION : ADD_PROJECT_MUTATION,
    {
      onCompleted: (data) => {
        openNotification(
          "topRight",
        ...
        onCompleted &&
          (onCompleted(data?.createProject || data?.updateProject) ||
            onCompleted());
      },
      onError: (error) => {
        openNotification(
          "topRight",
          "error",
          `Ошибка при выполнении сооздания : ${error.message}`
        );
      },
    }
  );
  ...
  const handleSave = () => {
    ...
    const facility_id =
      formData.facility_id?.checkedObjects?.map(
        (row) => row?.value[0] ?? null
      ) ?? project.facility_id;
    ...
    const data = {
      ...
      date_signing: formData?.dateStart
        ? dayjs(formData?.dateStart).format("YYYY-MM-DD")
        : null,
      duration: formData?.duration,
      date_end: formData.dateEnd
        ? dayjs(formData?.dateEnd).format("YYYY-MM-DD")
        : null,
  ...
  const createNumber = () => {
    if (type && project?.id && project?.id > -1) return project.number;

    const facilitiCode =
      (form.getFieldValue("facility_id")?.checkedObjects &&
        form.getFieldValue("facility_id")?.checkedObjects[0] &&
        form.getFieldValue("facility_id")?.checkedObjects[0]?.key) ??
      "__-__-___-___";
    const result =
      (dataGroupProject?.groupTypeProjects?.find(
        (row) => row.id === selectedGroupTypeProject
      )?.code ?? "___") +
      "–" +
      "24" +
      "–" +
      (projectCountByOrg.countProjectByOrganizations
        ? projectCountByOrg.countProjectByOrganizations
            .toString()
            ?.padStart(2, "0")
        : "__") +
      "–" +
      (form
        .getFieldValue("organization_customer")
        ?.selected?.padStart(3, "0") ?? "___") +
      "–" +
      facilitiCode;
    if (result.includes("_")) return null;
    return result;
  };
  ...
  if (errorStatuses || errorTypeProject || errorDelegates || errorOrganizations)
    return `Ошибка! ${
      errorStatuses?.message ||
      errorTypeProject?.message ||
      errorDelegates?.message ||
      errorOrganizations?.message
    }`;
        ...
        <>
          <Form form={form} onFinish={handleSave} layout="vertical">
            <Form.Item
              ...
              style={
                disabledOptions?.includes("name") ? { display: "none" } : {}
              }
            ...
            <AutoCompleteFormItem
              style={
                disabledOptions?.includes("type_project_document")
                  ? { display: "none" }
                  : {}
              }
              rulesValidationRequired={requiredOptions?.includes(
                "type_project_document"
              )}
              rulesValidationMessage={"Укажите тип проекта"}
              name="group_type_project_document"
              label="Тип документа"
            >
              <CustomAutoComplete
            ...
            <AutoCompleteFormItem
              ...
              label="Заказчик"
              style={
                disabledOptions?.includes("organization_customer")
                  ? { display: "none" }
                  : {}
              }
              rulesValidationRequired={requiredOptions?.includes(
            ...
            <Form.Item
              name="duration"
              style={
                disabledOptions?.includes("duration") ? { display: "none" } : {}
              }
              rules={[
                {
            ...
            <Form.Item
              ...
              style={
                disabledOptions?.includes("status_id")
                  ? { display: "none" }
                  : {}
              }
              rules={[
          ...
          <Modal
            key={
              delegatesModalStatus?.mode ||
              delegatesModalStatus?.organization_id ||
              null
            }
            ...
            children={
              <ContactForm
                cardProps={"Контакт"}
                onCompleted={() => setDelegatesModalStatus(null)}
                initialObject={
                  delegatesModalStatus?.organization_id
                    ? { id: delegatesModalStatus?.organization_id }
                    : null
                }
              />
            }
          ...
          <Modal
            key={
              organizationsModalStatus?.mode ||
              organizationsModalStatus?.organization_id ||
              null
            }
            ...
            children={
              <OrganizationForm
                ...
                onCompleted={(value) => {
                  ...
                  form.setFieldValue("organization_customer", {
                    selected: value.id,
                    output: value.name,
                  });
