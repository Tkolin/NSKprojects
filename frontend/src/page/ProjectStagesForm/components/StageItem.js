...
import { STAGES_QUERY_COMPACT } from "../../../graphql/queries/queriesCompact";
...
const StageItem = ({
  ...
  useEffect(() => {
    if (rowForm?.percent ?? null) handlePriceChange(rowForm?.percent);
  }, [rowForm?.percent]);
      ...
      <Space.Compact style={{ width: "100%" }}>
        ...
        <Col span={2}>
          <Tooltip title="Смещение">
            ...
            >
              <InputNumber
                max={project.duration - stageTimes.duration}
                onChange={(value) =>
                  setStageTimes({ ...stageTimes, offset: value })
                }
                placeholder={"Сроки смещение"}
                style={{ marginBottom: 0, width: "100%" }}
              />
            </Form.Item>
          </Tooltip>
        ...
        <Col span={2}>
          <Tooltip title="Укажите продолжнительность">
            <Form.Item
              ...
              name={[index, "duration"]}
              rules={[
                { required: true, message: "Укажите продолжнительность" },
              ]}
            >
              <InputNumber
                max={project.duration - stageTimes.offset}
                placeholder={"Сроки продолжнительность"}
                onChange={(value) =>
                  setStageTimes({ ...stageTimes, duration: value })
                }
                style={{ marginBottom: 0, width: "100%" }}
              />
            </Form.Item>
        ...
        <Col span={2}>
          <Tooltip title="Процент от общей стоимости">
            <Form.Item
              ...
              name={[index, "percent"]}
              rules={[
                { required: true, message: "Процент от стоимости обязателен" },
              ]}
            >
              <InputNumber
                max={100}
