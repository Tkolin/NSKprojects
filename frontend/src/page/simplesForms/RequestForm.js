...
import { REQUEST_QUERY_BY_ID } from "../../graphql/queries/queriesByID";
import {
...
const RequestForm = ({
  ...
  const [mutate, { loading: loadingSave }] = useMutation(
    actualObject ? UPDATE_REQUEST_MUTATION : ADD_REQUEST_MUTATION,
    {
      onCompleted: (data) => {
        openNotification(
          "topRight",
        ...
        form.resetFields();
        onCompleted &&
          onCompleted(data?.createRequests || data?.updateRequests);
      },
      onError: (error) => {
        openNotification(
          "topRight",
          "error",
          `Ошибка при выполнении создания заявки: ${error.message}`
        );
      },
    }
  );
