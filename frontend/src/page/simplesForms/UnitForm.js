...
const UnitForm = ({ localObject, initialObject, onCompleted, cardProps }) => {
  ...
  const [mutate, { loading: loadingSave }] = useMutation(
    actualObject ? UPDATE_UNIT_MUTATION : CREATE_UNIT_MUTATION,
    {
      onCompleted: (data) => {
        openNotification("topRight", "success", `Группа создана`);
        form.resetFields();
        onCompleted && onCompleted(data?.updateUnit || data?.createUnit);
      },
      onError: (error) => {
        openNotification(
          "topRight",
