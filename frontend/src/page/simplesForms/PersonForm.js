...
import { PERSONS_QUERY_BY_ID } from "../../graphql/queries/queriesByID";
import {
  BANKS_QUERY_COMPACT,
  BIKS_QUERY_COMPACT,
...
const PersonForm = ({ localObject, initialObject, onCompleted, cardProps }) => {
  ...
  const [mutate, { loading: loadingSave }] = useMutation(
    actualObject ? UPDATE_PERSON_MUTATION : ADD_PERSON_MUTATION,
    {
      onCompleted: (data) => {
        openNotification(
          "topRight",
          "success",
          `Создание новой записи в таблице ${nameModel} выполнено успешно`
        );
        form.resetFields();
      ...
      onError: (error) => {
        openNotification(
          "topRight",
          "error",
          `Ошибка при выполнении сооздания ${nameModel}: ${error.message}`
        );
      },
    }
  );
  ...
  useEffect(() => {
    if (initialObject?.id) loadContact();
  }, [initialObject]);
  useEffect(() => {
    if (localObject?.id) {
    ...
    if (data) {
      ...
      form.setFieldsValue({
        ...
        birth_date: data?.passport?.birth_date
          ? dayjs(data?.passport?.birth_date)
          : null,
  ...
  const {
    ...
    error: errorPpi,
    data: dataPpi,
  } = useQuery(PASSPORTS_PLACE_ISSUES_QUERY_COMPACT);
  const {
    loading: loadingBank,
    error: errorBank,
    data: dataBank,
  } = useQuery(BANKS_QUERY_COMPACT);
  const {
    loading: loadingBik,
    error: errorBik,
    data: dataBik,
  ...
  const handleSubmit = () => {
    ...
    const data = {
      ...
      address_registration:
        address?.registration?.unrestricted_value ??
        address?.registration ??
        null,
      address_residential:
        address?.residential?.unrestricted_value ??
        address?.residential ??
        null,
  ...
  if (errorPpi || errorBank || errorBik)
    return `Ошибка! ${
      errorPpi?.message || errorBank?.message || errorBik?.message
    }`;
          ...
          <Modal
            ...
            children={
              <PassportPlaceIssuesForm
                ...
                initialObject={
                  ppiModalStatus?.ppi_id ? { id: ppiModalStatus?.ppi_id } : null
                }
              />
            }
          />
