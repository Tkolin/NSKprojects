...
import {StyledBlockBig} from "./page/style/BlockStyles";
...
import {ADD_AND_UPDATE_TASK_MUTATION} from "./graphql/mutationsTask";
...
const TasksToProjectStageForm = ({project, onSubmit}) => {
    ...
    const handleSubmit = () => {
        const tasksNames = stagesArray.map(stage => stage.name);
        addTasksToProject({
                variables: {
                    names: tasksNames
                }
        ...
        const tasksToProject = extractDataFromTree(dataSource); //Перебрать иерархию задач по этапам (которые теперь тоже задачи)
